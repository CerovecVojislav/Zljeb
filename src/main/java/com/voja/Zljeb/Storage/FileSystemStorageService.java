...
public class FileSystemStorageService implements StorageService {
    ...
    public void store(MultipartFile file){
        try{
            if(file.isEmpty()){
                throw new StorageExceptions("Faled to strore empty file!");
            }
            Path destinationFile = this.rootLocation.resolve(
                Paths.get(file.getOriginalFilename())).normalize()
                .toAbsolutePath();
            if(!destinationFile.getParent().equals(this.rootLocation.toAbsolutePath())){
                throw new StorageExceptions("Cannot store file outside current directory!");
            }
    ...
    public Stream<Path> loadAll(){
        try{
            ...
            throw new StorageExceptions("Failed to read stored files: ", e);
        }
    }
